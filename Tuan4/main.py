...
class Graph:
    def __init__(self, polygons):
        ...
        for polygon in polygons:
            ...
            if polygon[0] == polygon[-1]:
                self.add_point(polygon[0]) # luu y
            else:
                for i, point in enumerate(polygon):
                    ...
                    if len(polygon) > 2:
                        ...
                        self.polygons[pid].add(edge) # Luu y
                    self.add_edge(edge)
                if len(polygon) > 2:
                    pid += 1

    def add_adjacent_points(self, point):
        return list(filter(None.__ne__, [edge.get_adjacent(point) for edge in self.edges])) # Luu y
    ...
    def can_see(self, start):
        ...
        for polygon in self.polygons:
            for edge in self.polygons[polygon]:
                for point in self.get_points(polygon):
                    if start == point:
    ...
    def add_edge(self, edge):
        self.graph[edge.p1].add(edge) # Chu y
    ...
    def h(self, point):
        heuristics = getattr(self, 'heuristics', None)
        if heuristics:
            return heuristics[point]
        else:
            return -1
...
def search(graph, start, goal, func):
    ...
    while not queue.empty():
        ...
        for i in graph.can_see(node):
            new_cost = node.g + euclid_distance(node, i) # Bi loi
